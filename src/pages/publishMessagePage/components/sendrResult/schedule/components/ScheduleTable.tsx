...
import BaseButton from "../../../../../../components/Atom/BaseButton.tsx";
import React from "react";
import {v4 as uuidv4} from "uuid"
import {formatPhoneNumber} from "../../../../../../apis/utils/formats/phoneNumberFormatUtil.ts";
import {useScheduleTable} from "../../../../../../components/hooks/customs/sendResult/sendReservation/useScheduleTable.ts";
...
export interface IScheduleTable {
  ...
  startPage: number;
  setSearchAllValue: React.Dispatch<any>
  listNumber: number
  pageSize: number
  eventHook: (pageNumber: number) => void
}

const ScheduleTable = (props: IScheduleTable) => {
  const scheduleTableHooks = useScheduleTable({props});
    ...
    <S.ScheduleTable>
      <form onSubmit={scheduleTableHooks.onClickSearch}>
        ...
        <div className="headFunWrap">
          ...
          <div>
            ...
            <S.SelectWrapper style={{width: '112px'}}>
              <button className={`${scheduleTableHooks.selectSwitch ? 'active' : null}`} type='button' onClick={(e) => {
                e.preventDefault()
                scheduleTableHooks.setSelectSwitch((prevState: any) => !prevState);
              }}>
                {scheduleTableHooks.searchSelectState.label}
              ...
              {
                scheduleTableHooks.selectSwitch ?
                  <S.SelectOptions>
                    {
                      scheduleTableHooks.typeSelect.map((item) => {
                        return (
                          <li key={item.value}>
                            <button type='button' onClick={() => {
                              scheduleTableHooks.handleSearchSelect(item)
                            }}>
                              {item.label}
                            </button>
                          </li>
        ...
        <div className="scheduleListWrap">
          ...
          <div className="scheduleBodyListWrap">
            {
              props.tableData[0]?.phoneNumber && (
                props.tableData?.map((el: any, index: number) => (
                  <ul className="scheduleBodyList" key={uuidv4()}>
                    {/*<li className="w10"><span>{ ((props.currentPage +1) * 10 - 10 ) + index + 1}</span></li>*/}
                    <li className="w10"><span>{props.listNumber + index + 1}</span></li>
                    <li className="w30">
                    ...
                    <li className="w30">
                      <span>{el?.phoneNumber ? formatPhoneNumber(el?.phoneNumber) : ""}</span>
                    </li>
                  </ul>
                ))
              )
          ...
          props.tableData[0]?.phoneNumber && (
            <div className="footFunWrap">
              ...
              <div className="reuseFunBtnWrap">
                <BaseButton onClick={(props.totalElements > 0 && !scheduleTableHooks.isUpdateLoading) ? scheduleTableHooks.onClickUpdateMsg : () => {
                }} width="112px" height="30px" fontSize="12px" fontWeight="600" backgroundColor="#0D42E5" borderRadius="50px" color="#fff">수정하기</BaseButton>
              </div>
            </div>
          )
        }
