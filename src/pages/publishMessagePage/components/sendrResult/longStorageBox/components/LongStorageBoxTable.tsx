...
import {useLongTable} from "../../../../../../components/hooks/customs/sendResult/long/useLongTable.ts";
...
const LongStorageBoxTable = ({setSearchAllValue, count, setActivePage, setStartPage, startPage, pageSize, activePage, eventHook, tableData, listNumber, detailInfo}: ISendResultTable) => {
  const lastPage = useMemo(() => Math.ceil((count === 0 ? pageSize : count ?? pageSize) / pageSize), [count, pageSize]);
  const useLongTableHooks = useLongTable({setSearchAllValue, activePage, lastPage, tableData, count, pageSize});
  const {reSendMsgToggleState, handleReSendMsg} = useReSendMsg();
  return (
    <S.SendResultTable>
      <form onSubmit={useLongTableHooks.onClickSearchButton}>
        ...
        <div className="headFunWrap">
          ...
          <div>
            ...
            <S.SelectWrapper style={{width: '112px'}}>
              <button className={`${useLongTableHooks.leftSelectSwitch ? 'active' : null}`} type='button' onClick={(e) => {
                e.preventDefault()
                useLongTableHooks.setLeftSelectSwitch(!useLongTableHooks.leftSelectSwitch)
              }}>
                {useLongTableHooks.keywordValueState.label}
              ...
              {
                useLongTableHooks.leftSelectSwitch ?
                  <S.SelectOptions>
                    {
                      useLongTableHooks.keywordValueArr.map((item) => {
                        return (
                          <li key={uuidv4()}>
                            <button type='button' onClick={() => {
                              useLongTableHooks.onChangeKeywordValue(item)
                              useLongTableHooks.setLeftSelectSwitch(false)
                            }}>
                              {item.label}
                            </button>
                          </li>
            ...
            <S.InputBox>
              <S.SearchInput
                ...
                value={useLongTableHooks.keywordState}
                onChange={(e: any) => {
                  useLongTableHooks.setKeywordState(e.currentTarget.value)
                }}
              />
              <BaseButton type={'submit'} backgroundColor="transition" onClick={useLongTableHooks.onClickSearchButton}>
        ...
        <div className="sendListWrap">
          <ul className="sendHeadList">
            ...
            <li className="w30"><strong>그룹명</strong></li>
            <li className="w30"><strong>이름</strong></li>
            <li className="w30"><strong>연락처</strong></li>
          </ul>
              ...
              tableData[0]?.phoneNumber && (
                tableData?.map((item: any, index: number) => (
                  <ul className="sendBodyList" key={uuidv4()}>
                    ...
                    <li className="w30">
                      <span>{item?.groupNm ?? "ㅡ"}</span>
                    </li>
                    <li className="w30">
                      <span>{item?.buddyNm ? item?.buddyNm : "ㅡ"}</span>
                    </li>
                    <li className="w30">
                      <span><span>{formatPhoneNumber(item?.phoneNumber)}</span></span>
                    </li>
                  </ul>
                ))
              )
          ...
          tableData[0]?.phoneNumber && (
            <div className="footFunWrap">
              ...
              <div className="reuseFunBtnWrap">
                <BaseButton width="112px" height="30px" fontSize="12px" fontWeight="600" backgroundColor={reSendMsgToggleState ? "#2D2D2F" : "#0D42E5"} borderRadius="50px" color="#fff"
                            onClick={() => handleReSendMsg(detailInfo)}
                >
                  {reSendMsgToggleState ? "재전송 취소" : "재전송"}
                </BaseButton>
                <BaseButton width="112px" height="30px" fontSize="12px" fontWeight="600" backgroundColor={'rgba(0, 0, 0, 0.8);'} borderRadius="50px" color="#fff"
                            onClick={useLongTableHooks.deleteMsgStorage}
                >
                  삭제
                </BaseButton>
              </div>
...
export default LongStorageBoxTable;