...
import LargeBtntopGroup from 'components/Molecules/addressbookLarge/LargeBtntopGroup';
import LargeBtnBottGroup from 'components/Molecules/addressbookLarge/LargeBtnBottGroup';

interface ILargeAddressTable {
...
const LargeAddressTable = ({
  ...
  return (
    <S.Container style={{ display: openLargeToggle ? '' : 'none' }}>
      <S.TableWrap>
        <S.TableInner>
          {/* <LargeBtntopGroup /> */}
          {groupListIsLoading ? (
            ...
            <S.TableSheet>
              <caption>대량 추가 테이블</caption>
              <S.TableHeadWrap>
                <S.TableHeaderRow>
                  {defaultRow?.map((row: any) => {
                    return (
                      <S.TableHeader
                        key={uuidv4()}
              ...
              <S.TableBodyWrap>
                {fields?.map((row: any, rowIndex: any) => {
                  return (
                    <LargeTableRow
                      key={row.id}
                      rowIndex={rowIndex}
