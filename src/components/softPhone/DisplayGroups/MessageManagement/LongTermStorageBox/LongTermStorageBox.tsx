...
import * as S from './LongTermStorageBox.styles'
import * as MHS from '../../MessageSendHistory/MessageSendHistory.styles'
import * as SBS from '../../styles/selectButton.styles'
import {ILongTermStorageBoxTypes} from "./LongTermStorageBox.types.ts";
import TopButton from "../TopButton/TopButton.index.tsx";
import { useLongTerm} from "../../../../hooks/customs/sendResult/long/useLongTerm.ts";
import {formatDateDate, formatDateTime} from "../../../../../apis/utils/formats/dateFormatUtil.ts";
import Loader from "../../../../common/Loader.tsx";


const LongTermStorageBox = ({handleChangeComponent}: ILongTermStorageBoxTypes) => {
// 장기보관함 관리 탭
  ...
  const longHooks = useLongTerm();


  return (
    <S.LongTermStorageBoxWrap>
      {longHooks.isLoading && (<Loader/>)}
      <TopButton handleChangeComponent={handleChangeComponent} focusNm={"long"}/>
      <div className="searchGroup">
        {/*	장기 보관함 조회 */}
        <SBS.Inquiry>
          <div className={`allSearchGroup ${isSearchActive ? 'active' : ''}`}>
            <button className="allsearchBtn" onClick={() => setIsSearchActive(prev => !prev)}>{longHooks.searchSelectMsgState.label} <span><svg xmlns="http://www.w3.org/2000/svg"
                                                                                                                                                                   width="14" height="6"
                                                                                                                                                                   viewBox="0 0 14 6" fill="none"><path
            ...
            <ul className='allSearchBox'>
              {longHooks.messageSelect.map((item: { value: string, label: string }) => (
                <li key={item.value} onClick={() => setIsSearchActive(false)}>
                  <button className='searchText' onClick={() => longHooks.handleSearchSelectMsg(item)}>{item.label}</button>
                </li>
              ))}
            </ul>
          </div>
          <div className="flexWrap">
            <div className={`contactGroup ${isContactActive ? 'active' : ''}`}>
              <button className="contactBtn" onClick={() => setIsContactActive(prev => !prev)}>{longHooks.searchSelectTypeState.label} <span><svg xmlns="http://www.w3.org/2000/svg"
                                                                                                                                                                      width="14" height="6"
                                                                                                                                                                      viewBox="0 0 14 6" fill="none"><path
                d="M12.9144 0.156925L7.44885 4.76826C7.32959 4.8694 7.16862 4.92613 7.00087 4.92613C6.83312 4.92613 6.67214 4.8694 6.55289 4.76826L1.08615 0.157925C0.965866 0.057316 0.804471 0.00100048 0.636419 0.00100048C0.468366 0.00100048 0.306971 0.057316 0.18669 0.157925C0.127602 0.207306 0.0806439 0.266326 0.0485811 0.331507C0.0165183 0.396689 0 0.466713 0 0.537453C0 0.608192 0.0165183 0.678216 0.0485811 0.743398C0.0806439 0.80858 0.127602 0.8676 0.18669 0.91698L5.6511 5.52732C6.01102 5.83027 6.49529 6 6.9997 6C7.50412 6 7.98838 5.83027 8.3483 5.52732L13.8127 0.91698C13.872 0.867584 13.9191 0.808502 13.9513 0.743228C13.9834 0.677954 14 0.607813 14 0.536953C14 0.466093 13.9834 0.395951 13.9513 0.330677C13.9191 0.265403 13.872 0.206321 13.8127 0.156925C13.6924 0.056316 13.531 0 13.363 0C13.1949 0 13.0335 0.056316 12.9133 0.156925"
                fill="#5B5B5C"/></svg></span></button>
              <ul className='contactBox'>
                {longHooks.typeSelect.map((item: { value: string, label: string }) => (
                  <li key={item.value} onClick={() => setIsContactActive(false)}>
                    <button className='contact' onClick={() => longHooks.handleSearchSelectType(item)}>{item.label}</button>
                  </li>
                ))}
            ...
            <div className="searchBox">
              <input type="text" placeholder='SEARCH' onChange={(e) => longHooks.handleSearchInput(e.currentTarget.value)} onKeyDown={longHooks.onClickSearch}/>
              <button className='searchIcon' onClick={longHooks.onClickSearch}>
      ...
      {/* 예약전송 관리 */}
      <S.LongTermStorageBox className='LongTermStorageBox'>
        <MHS.TransferList className='TransferList' ref={longHooks.listRef} onScroll={longHooks.onScrollHandler}>
          {
            longHooks.tableData[0]?.msgGroupId ? (
              longHooks.tableData.map((list: any) => (
                <div key={list.msgGroupId} className={`groups ${longHooks.longDetailParamsState.msgGroupId === list?.msgGroupId ? "active" : ""}`}
                     onClick={() => longHooks.handleActiveList(list?.msgGroupId, list?.callback)}>
                  <div className="titleGroup">
                    <h3>
                      <p className='messageTitle'>{list?.sndMsg ?? ""}</p>
                      {list?.imageData && (
                      ...
                      <p className='peopleNumber'>(<span className='peoleNum'>{list?.rcvPhnId ?? "0명"}</span><span>)</span></p>
                    </h3>
                    <div className="dateGroup">
                      <p className="date">{formatDateDate(list?.sndDttm)}</p>
                      <p className="time">{formatDateTime(list?.sndDttm)}</p>
                    </div>
                  </div>
                </div>
              ))
...
export default LongTermStorageBox;
