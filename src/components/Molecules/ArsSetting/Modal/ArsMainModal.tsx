...
import {useArsMainModal} from "../../../hooks/customs/say015/say105Handle/useArsMainModal.ts";

type ModalAction = '인사말' | '';
...
const ArsMainModal: React.FC<modalOpenProps> = ({mainModalToggle,setMainModalToggle,arsData, updatedJsonFile, node}) => {
  ...
  /* 안내멘트 모달창 Hooks */
  const {tapState, modalSave, audioPlayHooks, ttsHooks, recordHooks, clickOff, modalOff} = useArsMainModal({
  ...
  return (
    ...
    <S.SmsContainer id='smsContainer'>
      <S.SmsCover id='smsCover' onClick={clickOff}>
        <S.Title>{node.s015Data.action}</S.Title>
