...
import * as S from "./styles/AddressSearch.styles.ts"


import BaseButton from '../../Atom/BaseButton';

import { useAddressSearch } from "../../hooks/customs/addressBook/useAddressSearch.ts";

const AddressSearch = () => {

  const {
    onChangeSearchInput, onSubmitSearchPost, setSelectSwitch, selectSwitch, selectState, selectArrValue, onClickSelectListButton,
    searchState, onClickSearchButton, onChangeSelect
  } = useAddressSearch();
  return (
    <div>
      <form onSubmit={onSubmitSearchPost} aria-label="검색창">
        <S.SearchWrapper>
          <S.SelectWrapper>
            <button className={`${selectSwitch === true ? 'active' : null}`} type='button' onClick={(e) => {
              e.preventDefault()
          ...
          <S.InputBox>
            ...
            <BaseButton type={'submit'} backgroundColor="transition" onClick={onClickSearchButton}>
              ...
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="16" viewBox="0 0 17 17" fill="none">
