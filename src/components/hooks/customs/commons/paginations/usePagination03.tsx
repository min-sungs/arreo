...
export const usePagination03 = (args: IUsePaginationArgs): IUsePaginationReturns => {
  const { pageSize } = args;
  const lastPage = useMemo(
    () => Math.ceil((args.count === 0 ? pageSize : args.count ?? pageSize) / pageSize),
    [args.count, pageSize],
  );

  const onClickPage = useCallback((event: any): void => {
